...
public class traffic {
	//정확성 90.9/100
	 ...
	 public static int solution2(String[] lines) {
		 ...
		 int answer = 1;
		 int[][] table = new int[lines.length][2];
		 //시작 시간 구하기 완료
		 for(int i=0;i<lines.length;i++) {
			 String times = lines[i].split(" ")[1];
			 //시, 분을 계산한 초
			 table[i][1] =  (Integer.parseInt(times.split(":")[0])*3600+Integer.parseInt(times.split(":")[1])*60)*1000
					 +(int)((Double.parseDouble(times.split(":")[2]))*1000);
			 table[i][0] = table[i][1] - (int)(Double.parseDouble((lines[i].split(":")[2].split(" "))[1].split("s")[0]) *1000) + 1;
		 } 
		 // 수정해야함
		 int count;
		 for(int i=0;i<lines.length;i++) {
...
*/	 public static void main(String[] args) {
		...
		// String[] lines = {"2016-09-15 01:00:04.002 2.0s", "2016-09-15 01:00:07.000 2s"};
		 ...
		 System.out.println("결과 : "+solution2(lines));
	}
}
